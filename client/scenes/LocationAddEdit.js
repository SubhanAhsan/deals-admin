...
class LocationAddEdit extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.location.pathname === "/locations/add") { // if Add New
            ...
            let sourceUrl = location.hostname === 'localhost' ?
                'http://localhost:8080/api/locations/' + this.props.match.params.id :
                '/api/locations/' + this.props.match.params.id; //path based on environment (development/production)
    ...
    handleSave(event) {
        if (this.state.name === "") {
            this.setState({ snackbar: { open: true, message: "Please enter the location name" } });
            return false;
        }
