...
class Vendors extends Component {
  ...
  handleRowSelection = (selectedRows, evt) => {
    ...
    console.log(this.state.vendors[selectedRows]);
  };
  ...
  render() {
    var tableRows = this.state.vendors.map((data, index) => {
      //console.log("index " + index);
      return (
        <TableRow key={data._id} selected={this.isSelected({ index })}>
          <TableRowColumn>{data._id}</TableRowColumn>
          <TableRowColumn><Link to={'/vendors/' + data._id}>{data.name}</Link></TableRowColumn>
    ...
    return (
      <div id="sceneVendor" style={styles.scene}>
          ...
          <Paper zDepth={1}>
            <BottomNavigation >
              ...
              <BottomNavigationItem
                containerElement={<Link to="/vendors/add" />}
                icon={<IconAdd />}
                label="Add New"
              />
